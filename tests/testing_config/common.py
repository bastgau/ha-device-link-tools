...
from .custom_components.test_constant_deprecation import (
...
async def async_get_device_automations(
    ...
    automations = await device_automation.async_get_device_automations(hass, automation_type, [device_id])
    return automations.get(device_id)


def threadsafe_callback_factory(func) -> type:
...
def threadsafe_coroutine_factory(func) -> type:
    ...
    def threadsafe(*args, **kwargs):
        """Call func threadsafe."""
        hass = args[0]
        return asyncio.run_coroutine_threadsafe(func(*args, **kwargs), hass.loop).result()
...
class MockMqttReasonCode:
    ...
    def __init__(self, value: int = 0, is_failure: bool = False, name: str = "Success") -> None:
...
def async_fire_mqtt_message(
    ...
    from homeassistant.components.mqtt import MqttData  # noqa: PLC0415 # pylint: disable=import-outside-toplevel
...
@callback
def async_fire_time_changed(hass: HomeAssistant, datetime_: datetime | None = None, fire_all: bool = False) -> None:
...
@callback
def _async_fire_time_changed(hass: HomeAssistant, utc_datetime: datetime | None, fire_all: bool) -> None:
    """Fire time changed event for scheduled tasks."""
...
def get_fixture_path(filename: str, integration: str | None = None) -> pathlib.Path:
    """Get path of fixture."""
    if integration is None and "/" in filename and not filename.startswith(("core/", "helpers/")):
...
async def async_load_fixture(hass: HomeAssistant, filename: str, integration: str | None = None) -> str:
    """Load a fixture."""
    return await hass.async_add_executor_job(load_fixture, filename, integration)


def load_json_value_fixture(filename: str, integration: str | None = None) -> JsonValueType:
    """Load a JSON value from a fixture."""
    return json_loads(load_fixture(filename, integration))


def load_json_array_fixture(filename: str, integration: str | None = None) -> JsonArrayType:
...
def load_json_object_fixture(filename: str, integration: str | None = None) -> JsonObjectType:
    """Load a JSON object from a fixture."""
...
def mock_state_change_event(hass: HomeAssistant, new_state: State, old_state: State | None = None) -> None:
...
class RegistryEntryWithDefaults(er.RegistryEntry):
    ...
    options: er.ReadOnlyEntityOptionsType = attr.ib(default=None, converter=er._protect_entity_options)
...
def mock_area_registry(hass: HomeAssistant, mock_entries: dict[str, ar.AreaEntry] | None = None) -> ar.AreaRegistry:
...
class MockGroup(auth_models.Group):
    ...
    def __init__(self, id: str | None = None, name: str | None = "Mock Group") -> None: # pylint: disable=redefined-builtin
        """Mock a group."""
...
async def register_auth_provider(hass: HomeAssistant, config: ConfigType) -> auth_providers.AuthProvider:
    ...
    provider = await auth_providers.auth_provider_from_config(hass, hass.auth._store, config)
...
class MockModule:
    ...
    def __init__(
        ...
        *,
        ...
        async_setup: Callable[[HomeAssistant, ConfigType], Coroutine[Any, Any, bool]] | None = None,
        async_setup_entry: Callable[[HomeAssistant, ConfigEntry], Coroutine[Any, Any, bool]] | None = None,
        async_unload_entry: Callable[[HomeAssistant, ConfigEntry], Coroutine[Any, Any, bool]] | None = None,
        async_migrate_entry: Callable[[HomeAssistant, ConfigEntry], Coroutine[Any, Any, bool]] | None = None,
        async_remove_entry: Callable[[HomeAssistant, ConfigEntry], Coroutine[Any, Any, None]] | None = None,
...
class MockToggleEntity(entity.ToggleEntity):
    ...
    @property
    def state(self) -> Literal["on", "off"] | None: # pylint: disable=overridden-final-method
...
class MockConfigEntry(config_entries.ConfigEntry):
    ...
    async def start_reconfigure_flow(
        ...
        if self.entry_id not in hass.config_entries._entries:
            raise ValueError("Config entry must be added to hass to start reconfiguration flow")
...
def patch_yaml_files(files_dict, endswith=True) -> type:
...
@contextmanager
def assert_setup_component(count, domain=None) -> type:
    """Collect valid configuration from setup_component.

    Context manager for capturing setup component configuration.
    """
    config = {}
    ...
    async def mock_psc(
        ...
        integration_config_info = await async_process_component_config(hass, config_input, integration, component)
    ...
    if domain is None:
        assert len(config) == 1, f"assert_setup_component requires DOMAIN: {list(config.keys())}"
        domain = list(config.keys())[0]

    res = config.get(domain)
    res_len = 0 if res is None else len(res)
    assert res_len == count, f"setup_component failed, expected {count} got {res_len}: {res}"
...
def mock_restore_cache(hass: HomeAssistant, states: Sequence[State]) -> None:
    ...
    for state in states:
        ...
        restored_state = {
            **restored_state,
            "attributes": json.loads(json.dumps(restored_state["attributes"], cls=JSONEncoder)),
        }
        last_states[state.entity_id] = rs.StoredState.from_dict({"state": restored_state, "last_seen": now})
...
def mock_storage(data: dict[str, Any] | None = None) -> Generator[dict[str, Any]]:
    ...
    async def mock_write_data(store: storage.Store, data_to_write: dict[str, Any]) -> None:
...
async def get_system_health_info(hass: HomeAssistant, domain: str) -> dict[str, Any]:
    ...
    from homeassistant.components.system_health import (  # noqa: PLC0415 # pylint: disable=import-outside-toplevel
        DATA_SYSTEM_HEALTH_PLATFORMS,
        DOMAIN as SYSTEM_HEALTH_DOMAIN
    )

    platform_registrations = await hass.data[DATA_SYSTEM_HEALTH_PLATFORMS].async_get_platforms()
...
def async_mock_cloud_connection_status(hass: HomeAssistant, connected: bool) -> None:
    ...
    from homeassistant.components.cloud import (  # noqa: PLC0415 # pylint: disable=import-outside-toplevel
...
def import_and_test_deprecated_alias(
    ...
    *,
    ...
    """
    replacement_name = replacement_name or f"{replacement.__module__}.{replacement.__name__}"
...
def help_test_all(module: ModuleType) -> None:
    ...
    assert set(module.__all__) == {itm for itm in dir(module) if not itm.startswith("_")}
...
def get_quality_scale(integration: str) -> dict[str, QualityScaleStatus]:
    ...
    quality_scale_file = pathlib.Path(f"homeassistant/components/{integration}/quality_scale.yaml")
    if not quality_scale_file.exists():
        return {}
    raw = load_yaml_dict(quality_scale_file)
    return {
        rule: (QualityScaleStatus(details) if isinstance(details, str) else QualityScaleStatus(details["status"]))
...
def get_sensor_display_state(hass: HomeAssistant, entity_registry: er.EntityRegistry, entity_id: str) -> str:
    ...
    if (precision := entity_entry.options.get("sensor", {}).get("suggested_display_precision")) is None:
        return value
...
async def assert_platform_setup_creates_issue(
    ...
    assert (
        ...
        f" supported, it must be configured under its own {integration_domain} key instead" in caplog.text
    )
