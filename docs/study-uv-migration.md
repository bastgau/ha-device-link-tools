...
## The trap: `uv sync` is exact

`uv sync` makes the environment match the lock _exactly_: it removes anything the lock
does not list. Home Assistant's `default_config` integrations need 42 further packages
(`av`, `numpy`, `Pillow`, `SQLAlchemy`…) that only `scripts/develop` uses, and those pins
are deliberately kept out of the lock — they are generated by `scripts/update_requirements`
and live under `.devcontainer/`, out of Dependabot's reach.

Installed on top of the synced venv, they are exactly what a bare `uv sync` deletes. Hence
the policy:

| Consumer                          | Command                                                     | Why                                                                               |
| --------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `scripts/setup`                   | `uv sync`, **then** `uv pip install -r …default-config.txt` | the authoritative place: it prunes, then reinstalls, so it always ends consistent |
| `scripts/lint`, `test`, `develop` | `uv sync --frozen --inexact --quiet`                        | `--inexact` refuses to prune; `--frozen` refuses to rewrite the lock              |
